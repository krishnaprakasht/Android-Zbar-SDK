...
//zbar_config_t ZBAR_CFG_TEST_INVERTED;
...
static void symbol_handler(zbar_decoder_t *dcode)
{
    ...
    elapsed_time = zbar_decoder_get_time(dcode);
    ...
    sym->elapsed_time = zbar_decoder_get_time(dcode);
...
zbar_image_scanner_t *zbar_image_scanner_create()
{
    ...
    zbar_image_scanner_set_config(iscn, ZBAR_CODE11, ZBAR_CFG_UNCERTAINTY, 0);
...
static void *_zbar_scan_image(zbar_image_scanner_t *iscn, zbar_image_t *img)
{
    ...
    // Start image processing time  added by Heera//
    struct timespec t_start, t_end;
    // Capture start timestamp
    clock_gettime(CLOCK_REALTIME, &t_start);
    struct tm *tm_info_start = localtime(&t_start.tv_sec);
    char start_time_str[64];
    snprintf(start_time_str, sizeof(start_time_str), "%04d-%02d-%02d %02d:%02d:%02d.%03ld",
             tm_info_start->tm_year + 1900, tm_info_start->tm_mon + 1, tm_info_start->tm_mday,
             tm_info_start->tm_hour, tm_info_start->tm_min, tm_info_start->tm_sec,
             t_start.tv_nsec / 1000000);
    __android_log_print(ANDROID_LOG_ERROR, "img_scanner.c", "Edge detection start Timestamp: %s", start_time_str);
    //***************************************************************************************************************************//
...
// Capture end timestamp
    ...
    struct tm *tm_info_end = localtime(&t_end.tv_sec);
    char end_time_str[64];
    snprintf(end_time_str, sizeof(end_time_str), "%04d-%02d-%02d %02d:%02d:%02d.%03ld",
             tm_info_end->tm_year + 1900, tm_info_end->tm_mon + 1, tm_info_end->tm_mday,
             tm_info_end->tm_hour, tm_info_end->tm_min, tm_info_end->tm_sec,
             t_end.tv_nsec / 1000000);
    __android_log_print(ANDROID_LOG_ERROR, "img_scanner.c", "Image processing end Timestamp: %s", end_time_str);

// Calculate delta (execution time in milliseconds)
    double delta_ms = (t_end.tv_sec - t_start.tv_sec) * 1000.0 +
                      (t_end.tv_nsec - t_start.tv_nsec) / 1000000.0;
    __android_log_print(ANDROID_LOG_ERROR, "img_scanner.c", "Image processing Time delta: %.3f ms", delta_ms);
