...
static int validate_upca_guard(zbar_decoder_t *dcode) {
    // Validate the guard patterns for UPCA (start/middle/end)
    return (get_color(dcode) == ZBAR_BAR && dcode->ean.s4 > 0);
}

// Function to calculate UPCA checksum
static int upca_checksum(unsigned char *data, int length) {
    int sum = 0;
    for (int i = 0; i < length; i++) {
        sum += (i % 2 == 0) ? data[i] * 3 : data[i];
    }
    return (10 - (sum % 10)) % 10;
}

// Function to decode a UPCA part
static zbar_symbol_type_t upca_decode_part(zbar_decoder_t *dcode, ean_pass_t *pass) {
    int idx = pass->state & STATE_IDX;
    int code = decode4(dcode); // Decode a 4-bit segment
    if (code < 0) {
        return ZBAR_NONE; // Failed to decode
    }

    pass->raw[(idx >> 2) + 1] = digits[(unsigned char)code];
    if (idx == 0x0C) { // Check if it's the last segment
        int checksum = upca_checksum(pass->raw, 12);
        if (checksum == pass->raw[11]) {
            return ZBAR_UPCA; // Successfully decoded UPCA
        }
    }
    return ZBAR_NONE;
}
...
static inline zbar_symbol_type_t decode_pass(zbar_decoder_t *dcode, ean_pass_t *pass) {
        ...
        if (pass->state & STATE_ADDON) {
            ...
            if (idx == 0x09 || idx == 0x21) {
                ...
                if (part || idx == 0x21) {
                    ...
                    pass->state = -1;
                    __android_log_print(ANDROID_LOG_ERROR, "EAN.C", "Returning from 437  %d", part);
                    return part;
                }
            }
            if ((idx & 7) == 1) {
            ...
            pass->state = -1;
            __android_log_print(ANDROID_LOG_ERROR, "EAN.C", "Returning from 488   %d", part);
            return part;
        }
            // Handle UPCA decoding
        else if ((idx == 0x20 || idx == 0x21) &&
                 TEST_CFG(dcode->ean.upca_config, ZBAR_CFG_ENABLE)) {
            if (!validate_upca_guard(dcode)) {
                return ZBAR_NONE; // Invalid UPCA guard pattern
            }

            zbar_symbol_type_t part = upca_decode_part(dcode, pass);
            if (part) {
                dcode->ean.direction = (pass->state & STATE_REV) != 0;
                pass->state = -1;
                __android_log_print(ANDROID_LOG_ERROR, "EAN.C", "Returning UPCA decoded part: %d", part);
                return part;
            }
        }
            // Handle additional logic for other EAN types
            ...
            if (part)
                dcode->ean.direction = (pass->state & STATE_REV) != 0;
            pass->state = -1;
            __android_log_print(ANDROID_LOG_ERROR, "EAN.C", "Returning from 515   %d", part);
            return part;
        }
    }

    // Decode numeric digits for EAN and UPCA
    if (pass->state & STATE_ADDON)
        idx >>= 1;
...
static inline void ean_expand_upce(ean_decoder_t *ean, ean_pass_t *pass)
{
    ...
    // Ensure valid input to prevent crashes
    if (!ean || !pass || !pass->raw) {
        __android_log_print(ANDROID_LOG_ERROR, "ean_expand_upce", "Invalid input data");
        return;
    }
    ...
    // Debugging: Print all 12 values in ean->buf
                                                    ...
                                                    zbar_symbol_type_t part)
{
    ...
    __android_log_print(ANDROID_LOG_ERROR, "EAN.C", "PART ------------- %d", part);
...
{
    //__android_log_print(ANDROID_LOG_ERROR, "EAN.C", "_zbar_decode_ean" );
    ...
    for(i = 0; i < 4; i++) {
           ...
           i == pass_idx)
        {
            ...
            if(part) {
                ...
                if(sym) {
                    //__android_log_print(ANDROID_LOG_ERROR, "EAN.C", "SYM ================ %d", sym);
                    ...
                    if(sym > ZBAR_PARTIAL) {
                       ...
                       LOG_PRINT("Chiru: Time taken for decoding ean barcode = %.4f ms",elapsed_time);
