...
#include <zbar.h>
    ...
    defined(DEBUG_DATABAR) || defined(DEBUG_CODE128) || defined(DEBUG_CODE11) || \
...
zbar_decoder_t *zbar_decoder_create()
{
    ...
    dcode->ean.ean8_config = ((1 << ZBAR_CFG_ENABLE) |
                              (1 << ZBAR_CFG_EMIT_CHECK));
    dcode->ean.upca_config = ((1 << ZBAR_CFG_ENABLE) | (1 << ZBAR_CFG_EMIT_CHECK));
    dcode->ean.upce_config =  ((1 << ZBAR_CFG_ENABLE) | (1 << ZBAR_CFG_EMIT_CHECK));
...
#endif
#if ENABLE_CODE11 == 1
    dcode->code11.config		 = 1 << ZBAR_CFG_ENABLE;
    CFG(dcode->code11, ZBAR_CFG_MIN_LEN) = 1;
#endif
#if ENABLE_CODE93 == 1
...
const char *zbar_decoder_get_time(const zbar_decoder_t *dcode)
{
    return ((char *)dcode->elapsed_time);
}
...
#endif
#if ENABLE_CODE11 == 1
    if (TEST_CFG(dcode->code11.config, ZBAR_CFG_ENABLE) &&
        (tmp = _zbar_decode_code11(dcode)) > ZBAR_PARTIAL)
        sym = tmp;
#endif
#if ENABLE_CODE93 == 1
